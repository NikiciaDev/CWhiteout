...
Timer::Timer(const std::string name, const mdl::MODULE_CATEGORY category) : Module(name, category, 'G') {}

void Timer::on_enable(std::map<const std::string, JavaClass*>& classes) {
	jenv_ptr->SetFloatField(classes.find("Timer")->second->instance, *classes.find("Timer")->second->jfields.find("timer_speed")->second, speed.value);
}
