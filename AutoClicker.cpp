...
void AutoClicker::on_call(std::map<const std::string, JavaClass*>& classes) {
	if (GetAsyncKeyState(VK_LBUTTON) < 0 && left_cps.value > 0) {
		short s = left_cps.value + rul::rndm_num_ir<short>(-(this->random_left.value), this->random_left.value, clock.start_count());
	...
	if (GetAsyncKeyState(VK_RBUTTON) < 0 && right_cps.value > 0) {
		short s = right_cps.value + rul::rndm_num_ir<short>(-(this->random_right.value), this->random_right.value, clock.start_count());
