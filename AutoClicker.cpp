...
void AutoClicker::on_call(std::map<const std::string, JavaClass*>& classes) {
	if (GetAsyncKeyState(VK_LBUTTON) < 0 && this->random_left.gv<short>() > 0) {
		if (delay.ii()) {
			delay.sv(this->left_cps.gv<short>() + rndm.intg<short>(-(this->random_left.gv<short>()), this->random_left.gv<short>()));
		}

		if (delay.gv() <= 0) delay.sv(std::any_cast<short>(left_cps.gv<short>()));
	...
	if (GetAsyncKeyState(VK_RBUTTON) < 0 && this->random_right.gv<short>() > 0) {
		if (delay.ii()) {
			delay.sv(this->random_right.gv<short>() + rndm.intg<short>(-(this->random_right.gv<short>()), this->random_right.gv<short>()));
		}

		if (delay.gv() <= 0) delay.sv(this->random_right.gv<short>());
		if (clock.has_time_elapsed(std::chrono::milliseconds(1000 / delay.gv()))) {
